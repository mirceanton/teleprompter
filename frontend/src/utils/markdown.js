...
export function getCurrentSection(sections, currentLine) {
  return (
    sections.find(
      (section) => currentLine >= section.start && currentLine <= section.end
    ) || null
  );
}
...
export function getPreviousSection(sections, currentLine) {
  ...
  if (!currentSection) {
    ...
    const sectionsBeforeLine = sections.filter(
      (section) => section.start < currentLine
    );
    return sectionsBeforeLine.length > 0
      ? sectionsBeforeLine[sectionsBeforeLine.length - 1]
      : null;
  }
